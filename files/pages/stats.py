...
st.title("Statistics Mentor")
st.caption(f"Topic: {t} | Level: {l}")

if "chat_st" not in st.session_state or "message_st" not in st.session_state:
    st.session_state["chat_st"] = []
    st.session_state["message_st"] = []
    st.session_state["message_st"].append(("system","""You are an expert assistant restricted
...
for i in st.session_state["chat_st"]:
    with st.chat_message(i["role_st"]):
        st.write(i["content"])
...
if user_input:
    ...
    st.session_state["message_st"].append(("user",final_prompt))
    st.session_state["chat_st"].append({"role":"user","content":user_input})
    ...
    output = cm.invoke(st.session_state["message_st"])
    with st.chat_message("ai"):
        st.write(output.content)

    st.session_state["chat_st"].append({"role":"ai","content":output.content})
    st.session_state["message_st"].append(("ai",output.content))

def save_chat_as_txt(chat_history, topic, level, filename="ai_mentor_chat.txt"):
    with open(filename, "w", encoding="utf-8") as f:
        f.write("Statistics Mentor Conversation\n")
...
if st.session_state["chat_st"]:
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        st.download_button(
            ...
            data=f,
            file_name="Statistics_Mentor_Chat.txt",
            mime="text/plain"
        )
