...
if "chat_ml" not in st.session_state or "message_ml" not in st.session_state:
    st.session_state["chat_ml"] = []
    st.session_state["message_ml"] = []
...
for i in st.session_state["chat_ml"]:
    with st.chat_message(i["role_ml"]):
        st.write(i["content"])
...
if user_input:
    ...
    st.session_state["message_ml"].append(("user",final_prompt))
    st.session_state["chat_ml"].append({"role":"user","content":user_input})
    ...
    output = cm.invoke(st.session_state["message_ml"])
    with st.chat_message("ai"):
        st.write(output.content)

    st.session_state["chat_ml"].append({"role":"ai","content":output.content})
    st.session_state["message_ml"].append(("ai",output.content))

def save_chat_as_txt(chat_history, topic, level, filename="ai_mentor_chat.txt"):
    with open(filename, "w", encoding="utf-8") as f:
        f.write("Machine Learning Mentor Conversation\n")
...
if st.session_state["chat_ml"]:
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        st.download_button(
            ...
            data=f,
            file_name="Machine_Learning_Mentor_Chat.txt",
            mime="text/plain"
        )
