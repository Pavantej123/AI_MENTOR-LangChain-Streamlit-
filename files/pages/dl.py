...
st.title("Deep Learning Mentor")
st.caption(f"Topic: {t} | Level: {l}")

if "chat_dl" not in st.session_state or "message_dl" not in st.session_state:
    st.session_state["chat_dl"] = []
    st.session_state["message_dl"] = []
    st.session_state["message_dl"].append(("system","""You are an expert assistant restricted
...
for i in st.session_state["chat_dl"]:
...
if user_input:
    ...
    st.session_state["message_dl"].append(("user",final_prompt))
    st.session_state["chat_dl"].append({"role":"user","content":user_input})
    ...
    output = cm.invoke(st.session_state["message_dl"])
    with st.chat_message("ai"):
        st.write(output.content)

    st.session_state["chat_dl"].append({"role":"ai","content":output.content})
    st.session_state["message_dl"].append(("ai",output.content))

def save_chat_as_txt(chat_history, topic, level, filename="ai_mentor_chat.txt"):
    with open(filename, "w", encoding="utf-8") as f:
        f.write("Deep Learning Mentor Conversation\n")
...
if st.session_state["chat_dl"]:
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        st.download_button(
            ...
            data=f,
            file_name="Deep_Learning_Mentor_Chat.txt",
            mime="text/plain"
        )
