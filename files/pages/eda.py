...
if "chat_ed" not in st.session_state or "message_ed" not in st.session_state:
    st.session_state["chat_ed"] = []
    st.session_state["message_ed"] = []
    st.session_state["message_ed"].append(("system","""You are an expert assistant restricted
...
for i in st.session_state["chat_ed"]:
...
if user_input:
    ...
    st.session_state["message_ed"].append(("user",final_prompt))
    st.session_state["chat_ed"].append({"role":"user","content":user_input})
    ...
    output = cm.invoke(st.session_state["message_ed"])
    with st.chat_message("ai"):
        st.write(output.content)

    st.session_state["chat_ed"].append({"role":"ai","content":output.content})
    st.session_state["message_ed"].append(("ai",output.content))

def save_chat_as_txt(chat_history, topic, level, filename="ai_mentor_chat.txt"):
    with open(filename, "w", encoding="utf-8") as f:
        f.write("EDA Mentor Conversation\n")
