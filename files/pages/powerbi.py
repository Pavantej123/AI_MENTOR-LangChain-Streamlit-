...
st.title("Power BI Mentor")
st.caption(f"Topic: {t} | Level: {l}")

if "chat_pb" not in st.session_state or "message_pb" not in st.session_state:
    st.session_state["chat_pb"] = []
    st.session_state["message_pb"] = []
    st.session_state["message_pb"].append(("system","""You are an expert assistant restricted
...
for i in st.session_state["chat_pb"]:
...
if user_input:
    ...
    st.session_state["message_pb"].append(("user",final_prompt))
    st.session_state["chat_pb"].append({"role":"user","content":user_input})
    ...
    output = cm.invoke(st.session_state["message_pb"])
    with st.chat_message("ai"):
        st.write(output.content)

    st.session_state["chat_pb"].append({"role":"ai","content":output.content})
    st.session_state["message_pb"].append(("ai",output.content))

def save_chat_as_txt(chat_history, topic, level, filename="ai_mentor_chat.txt"):
    with open(filename, "w", encoding="utf-8") as f:
        f.write("PowerBi Mentor Conversation\n")
...
if st.session_state["chat_pb"]:
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        st.download_button(
            ...
            data=f,
            file_name="PowerBi_Mentor_Chat.txt",
            mime="text/plain"
        )
